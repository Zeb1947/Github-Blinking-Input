    ...
    while cap.isOpened():
        ...
        if results.multi_face_landmarks:
            ...
            for pts in [left_eye, right_eye]:
                ...
                overlay_pts = np.int32(pts)
                overlay[overlay_pts[:,1], overlay_pts[:,0]] = (0,200,255)  # fast dots

            # Face bounding box
            x,y,w_,h_ = cv2.boundingRect(np.int32(mesh))
            cv2.rectangle(overlay, (x,y), (x+w_, y+h_), (255,0,0), 2)
            ...
            bar_x = w - 50
            bar_top = int(h*(1-ear))
            cv2.rectangle(overlay, (bar_x, bar_top), (bar_x+25, h-10), color, -1)
            cv2.rectangle(overlay, (bar_x, int(h*(1-threshold))), (bar_x+25, h-10), (255,255,255), 2)
        ...
        put_text(overlay, f"TRACKER {'ON' if blink_enabled else 'OFF'}", (w-200, 30), toggle_color, 0.8)
...
def get_settings_from_gui(baseline):
    ...
    def on_launch():
        ...
        except: settings = default_settings
        root.destroy()

    root = tk.Tk()
    root.title("Eye Tracker Launcher")
    root.geometry("400x260")
    root.resizable(False, False)

    tk.Label(root,text="Eye Tracker Configuration",font=("Arial",14,"bold")).pack(pady=10)
